...
import {
    ModuleKind, ScriptTarget, Program, TypeChecker, SyntaxKind,
    SymbolFlags, SymbolTable, Declaration, Type, JSDocTypeTag, JSDoc, JSDocType
} from 'typescript';
...
class TypescriptParser {
    ...
    constructor() {
        ...
        this.program = typescript.createProgram(["node_modules/ant-design-vue/types/index.d.ts"], {
            rootNames: [],
            options: {
    ...
    folderFixDict = { /*"auto-complete": "autoComplete" */ };
    run() {
        ...
        for (var source of program.getSourceFiles()) {
            if (source.fileName.indexOf("node_modules/ant-design-vue/") < 0) { continue; }
            if (this.excludes.find(c => source.fileName.endsWith(c))) { continue; }
            ...
            for (var exportKey of exportKeys) {
                ...
                var members = this.getMembers(antdTag, symbol.exports.get(exportKey).members);
    ...
    async writeUITagsJsFile(memberTable) {
        var templateJson: any = await file.readFile("./build/ui-tags-template.json");
        ...
        for (var key in memberTable) {
            ...
            if (!template[key]) {
                template[key] = { description: key.substr(2, key.length - 1).split("-").join(" ") }
            }
            template[key].attributes = attributes;
        }
        var jsonFilePath = "./src/config/ui-tags.json";
        file.writeFile(jsonFilePath, JSON.stringify(template, null, "\t"), (err) => {
            if (err) {
    ...
    writeUIAttributesJsFile(fileList: { fileName: any; folder: any; }[]) {
        ...
        for (var item of fileList) {
            var variableName = item.fileName == "switch" ? "switchJson" : item.fileName;
            defineArray.push(`import * as ${variableName} from './attributes/${item.folder}/${item.fileName}.json';`);
...
${exportArray.join("\n")}
        ...
        if (t.types && t.types.length > 1) {
            ...
            if (set.size == 1) {
                if (typeString.indexOf("\"") >= 0) {
                    return { type: "string", options: JSON.parse(`[${typeString.replace(/\|/ig, ",")}]`) }
                }
    ...
    getMembers(prefix: string, members: SymbolTable): any {
        ...
        for (var key of this.getKeys(members)) {
            ...
            var result: def.TagAttributes = {
                ...
                optionType: simpleType.type == "any" && comment.type ? comment.type : simpleType.type,
    ...
    getJsDocDefaultValue(jsDoc: JSDoc) {
        if (jsDoc.tags) {
            var defaults = jsDoc.tags.filter(c => c.tagName && c.tagName.escapedText == "default");
    ...
    getJsDocType(jsDoc: any) {
        var type = "";
        if (jsDoc && jsDoc.tags) {
            var typeTags = jsDoc.tags.filter(c => c.kind === SyntaxKind.JSDocTypeTag);
    ...
    getMemberComment(declaration: Declaration): { comment: string, default: string, type: string } {
        var jsDoc = (declaration as any).jsDoc;
        var comment = "-", defaultValue = "-", type = "";
